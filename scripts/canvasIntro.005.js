...
canvasApp.push( function() { 
			...
			if (typeof ctrl.p === 'undefined') { 
				...
				ctrl.drawScreen = function drawScreen(w, h) { 
					...
					for (var i=2,z=vpoints.length; i<z; i+=2) { 
						...
						c.bezierCurveTo(vpoints[i-1][1], vpoints[i-1][2], vpoints[i][1], vpoints[i][2], vpoints[i][1], vpoints[i][2]); 
					} 
					c.lineWidth = 2; 
					c.strokeStyle=fade; 
					c.stroke(); 
					...
					for (var p=0, endp=vpoints.length; p<endp; p++) { 
						...
						if (mouse_up && (hXY[0] !== 0)) { 
							...
							if (hit && (vpoints[p][0] === 0) && (vpoints[(p+1)] !== 'undefined') && (vpoints[(p+1)][0] === 1)) { 
	...
	if ('ontouchmove' in document.createElement('div')) { 
		/*
		win.bind('touchstart', function(e){ e.preventDefault(); }); 
		win.bind('touchmove', function(e){ e.preventDefault(); }); 
		win.bind('touchend', function(e){ e.preventDefault(); });
		*/ 
